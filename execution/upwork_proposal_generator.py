...
def summarise_description(job: dict, anthropic_client: anthropic.Anthropic) -> str:
    """Use Haiku to generate a 1-2 sentence summary of the job description."""
    description = job.get('description', '')
    if not description:
        return ""
    try:
        response = anthropic_client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": (
                    f"Summarise this Upwork job in 1-2 sentences. Be concise and factual — "
                    f"what does the client want built/done?\n\n{description[:1500]}"
                )
            }]
        )
        return response.content[0].text.strip()
    except Exception as e:
        print(f"    Summary failed: {str(e)[:40]}")
        return ""
...
def generate_cover_letter(job: dict, proposal_doc_url: str, client: anthropic.Anthropic) -> str:
    """Generate customized cover letter using Sonnet."""
...
def generate_proposal(job: dict, client: anthropic.Anthropic, contact_info: dict = None) -> str:
    ...
    prompt = f"""Write a personalized project proposal for this Upwork job. Write as Lewis - first person, conversational, direct.
...
def process_job(job: dict, anthropic_client, drive_service, docs_service) -> dict:
    ...
    # Summarise description (uses Haiku)
    description_summary = summarise_description(job, anthropic_client)

    # Discover contact name first (uses Haiku)
...
def push_to_airtable(jobs: list[dict]) -> int:
    """Push processed jobs to Airtable Upwork Jobs table. Skips duplicates by Job URL."""
    import os
    api_key = os.getenv("AIRTABLE_API_KEY")
    base_id = os.getenv("AIRTABLE_BASE_ID")
    table_id = os.getenv("AIRTABLE_UPWORK_ID")

    if not all([api_key, base_id, table_id]):
        print("Airtable env vars missing (AIRTABLE_API_KEY, AIRTABLE_BASE_ID, AIRTABLE_UPWORK_ID) — skipping")
        return 0
    ...
    # Fetch existing Job URLs to deduplicate
    existing_urls = set()
    offset = None
    while True:
        params = {"fields[]": "Job URL", "pageSize": 100}
        if offset:
            params["offset"] = offset
        resp = requests.get(base_url, headers=headers, params=params)
        data = resp.json()
        for record in data.get("records", []):
            url = record.get("fields", {}).get("Job URL", "")
            if url:
    ...
    print(f"Airtable: {len(existing_urls)} existing records found")
    ...
    for job in jobs:
        ...
        # Normalise to Airtable single select options
        exp_map = {"ENTRY_LEVEL": "Entry", "INTERMEDIATE": "Intermediate", "EXPERT": "Expert"}
        exp_level = exp_map.get(exp_level.upper(), exp_level.title()) if exp_level else ""
        ...
        fields = {
            ...
            "Client Country": client.get("country", "") if isinstance(client, dict) else "",
            "Connects Cost": job.get("connects_cost", 0) or 0,
            "Cover Letter": job.get("cover_letter", ""),
            "Proposal Doc": job.get("proposal_doc", ""),
            "Status": "Not Applied",
            "Scraped At": scraped_at,
        }

        # Optional fields — only include if values exist
        if exp_level:
            fields["Experience Level"] = exp_level
        if isinstance(client, dict) and client.get("total_spent"):
            fields["Client Total Spent"] = float(client["total_spent"])
        if isinstance(client, dict) and client.get("total_hires"):
            fields["Client Total Hires"] = int(client["total_hires"])
        if job.get("contact_name"):
            fields["Contact Name"] = job["contact_name"]
        if job.get("contact_confidence"):
            fields["Contact Confidence"] = job["contact_confidence"]
        if job.get("description_summary"):
            fields["Summary"] = job["description_summary"]
        ...
        if resp.ok:
            ...
            print(f"  Airtable error for '{job.get('title', '')[:40]}': {resp.text[:100]}")

    print(f"Airtable: {created} created, {skipped} skipped (duplicates)")
    return created
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if error:
                print(f"[{completed}/{len(jobs)}] FAIL {jobs[idx]['title'][:40]}... Error: {error[:50]}", flush=True)
            else:
    ...
    # Push to Airtable
    push_to_airtable(processed_jobs)
